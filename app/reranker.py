...
def rerank_texts(query: str, texts: List[str]) -> List[str]:
    """
    ...
    # Assuming response: {"results": [{"document": {...}, "relevance_score": 0.95}, ...]}
    if "results" not in data:
        raise RuntimeError("Invalid Jina reranker response structure.")

    results = data["results"][: _config.top_k]

    # Extract the text from each result, assuming it's in "document" field or similar
    reranked_texts = []
    for result in results:
        ...
        if isinstance(doc, dict):
            ...
            text = str(doc)
        reranked_texts.append(text.strip())
